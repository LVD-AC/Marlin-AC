...
static void clean_up_after_endstop_or_probe_move() {
   ...
   */
  float probe_pt(const float &rx, const float &ry, const bool stow, const uint8_t verbose_level, const bool relative=true) {
    #if ENABLED(DEBUG_LEVELING_FEATURE)
      if (DEBUGGING(LEVELING)) {
    ...
    const float nx = rx - (relative ? (X_PROBE_OFFSET_FROM_EXTRUDER) : 0.0),
                ny = ry - (relative ? (Y_PROBE_OFFSET_FROM_EXTRUDER) : 0.0);
    ...
    if (!DEPLOY_PROBE()) {
      measured_z = run_z_probe() + (relative ? zprobe_zoffset : 0.0);
...
inline void gcode_G4() {
  ...
  inline bool home_delta() {
    ...
    // This can occur if the delta height is not set correctly.
...
void home_all_axes() { gcode_G28(true); }
   ...
   */
  static void print_calibration_results(const float z_at_pt[NPP + 1], const bool tower_points, const bool opposite_points) {
    SERIAL_PROTOCOLPGM(".    ");
  ...
  static float probe_calibration_points(float z_at_pt[NPP + 1], const int8_t probe_points, const bool towers_set, const float o_fac, const bool stow_after_each) {
    ...
    if (!_0p_calibration) {
      ...
      LOOP_CAL_ALL(rad) z_at_pt[rad] -= o_fac;
  ...
  static void forward_kinematics_probe_points(float mm_at_pt_axis[NPP + 1][ABC], float z_at_pt[NPP + 1]) {
  ...
  static void calc_kinematics_diff_probe_points(float z_at_pt[NPP + 1], float delta_e[ABC], float delta_r, float delta_t[ABC]) {
    const float z_center = z_at_pt[CEN];
    float diff_mm_at_pt_axis[NPP + 1][ABC],
          new_mm_at_pt_axis[NPP + 1][ABC];

    reverse_kinematics_probe_points(z_at_pt, diff_mm_at_pt_axis);
    ...
    LOOP_XYZ(axis) {
      diff_mm_at_pt_axis[CEN][axis] -= new_mm_at_pt_axis[CEN][axis] + delta_e[axis];
      LOOP_CAL_RAD(rad) diff_mm_at_pt_axis[rad][axis] -= new_mm_at_pt_axis[rad][axis] + delta_e[axis];
    }  
    LOOP_XYZ(axis) {
      diff_mm_at_pt_axis[CEN][axis] = -diff_mm_at_pt_axis[CEN][axis];
      LOOP_CAL_RAD(rad) diff_mm_at_pt_axis[rad][axis] = -diff_mm_at_pt_axis[rad][axis];
    }  
    forward_kinematics_probe_points(diff_mm_at_pt_axis, z_at_pt);

    LOOP_CAL_RAD(rad) z_at_pt[rad] -= z_at_pt[CEN] - z_center;
    z_at_pt[CEN] = +z_center;
  ...
  static float calc_kinematics_probe_points(float z_at_pt[NPP + 1], float delta_e[ABC], float delta_r, float delta_t[ABC]) {
    float diff_z_at_pt[NPP + 1];

    diff_z_at_pt[CEN] = z_at_pt[CEN];
    LOOP_CAL_RAD(rad) diff_z_at_pt[rad] = z_at_pt[rad];
    calc_kinematics_diff_probe_points(diff_z_at_pt, delta_e, delta_r, delta_t);
    z_at_pt[CEN] -= diff_z_at_pt[CEN];
    LOOP_CAL_RAD(rad) z_at_pt[rad] -= diff_z_at_pt[rad];
  ...
  static float auto_tune_a() {
    ...
    LOOP_XYZ(axis) {
      ...
      delta_t[axis] = diff;
      calc_kinematics_diff_probe_points(z_at_pt, delta_e, delta_r, delta_t);     
      a_fac -= z_at_pt[uint8_t((axis * _4P_STEP) - _7P_STEP + NPP) % NPP + 1] / 6.0;
   ...
   *      P0     Calibrates height only without probing.
  ...
  inline void gcode_G33() {
    ...
    do { // start iterations
      ...
      // Probe the points
      zero_std_dev_old = zero_std_dev;
      zero_std_dev = probe_calibration_points(z_at_pt, probe_points, towers_set, o_factor, stow_after_each);
      if (isnan(zero_std_dev)) {
        SERIAL_PROTOCOLPGM("Correct delta settings with M665 and M666");
        SERIAL_EOL();
        return AC_CLEANUP();
      }
      ...
      if ((zero_std_dev < test_precision || iterations <= force_iterations) && zero_std_dev > calibration_precision) {
        ...
        do { // iterate with kinematics
          ...
          sub_std_dev = calc_kinematics_probe_points(calc_z_at_pt, e_delta, r_delta / divider, t_delta);
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  inline bool prepare_kinematic_move_to(float ltarget[XYZE]) {
    ...
    if (!position_is_reachable(ltarget[X_AXIS], ltarget[Y_AXIS])) return true;
