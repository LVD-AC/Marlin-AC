...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G33() {
    ...
    if (!_1p_calibration && !_0p_calibration) { // test if the outer radius is reachable
      LOOP_CAL_RAD(axis) {
        ...
        if (!position_is_reachable(cos(a) * r, sin(a) * r)) {
          SERIAL_PROTOCOLPGM("?(M665 B)ed radius is implausible.");
          SERIAL_EOL();
          return;
        }
      }
    ...
    #if !HAS_BED_PROBE
      if (_Zo_calibration) {
        SERIAL_PROTOCOLPGM("?A probe is needed to calibrate the Z-offset.");
        SERIAL_EOL();
        return;
      }
    #endif
    ...
    do { // start iterations
      ...
      if ((zero_std_dev < test_precision || iterations <= force_iterations) && zero_std_dev > calibration_precision) {
        ...
        // calculate factors
        if (_Zo_calibration) {
          zprobe_zoffset = lcd_probe_pt(0, 0) + cal_ref - z_at_pt[CEN];
        }
