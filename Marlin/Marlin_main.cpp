...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G30() {
    ...
    /**
     * G33 - Delta '1-4-7-13 point' Auto-Calibration
     ...
     *      P1     Probe 1 point at center and set height only.
     *      P2     Probe 4 points: center and towers. Set height, endstops, and delta radius.
     *      P3     Probe 7 points: center, towers and opposite towers. Set all.
     *      P4-P7  Probe 13 points at different locations and average them. Set all.
     *
     ...
     *   
     *   A1  Auto tune calibration factors
     */
    ...
    void print_G33_results(float z_at_pt[13], const int8_t probe_points, const bool towers_set, const int8_t verbose_level){
      const bool _4p_calibration      = probe_points == 2,
      ...
      if (_7p_intermed_points)
        for (uint8_t axis = 1; axis < 13; axis += 2)  // average intermediates to tower and opposites
    ...
    void probe_G33_points(float z_at_pt[13], const int8_t probe_points, const bool towers_set, const bool stow_after_each){
      ...
      if (!_0p_calibration) {
        ...
        if (_7p_calibration) { // probe extra center points
          ...
          z_at_pt[0] /= float(_7p_double_circle ? probe_points + 2 : probe_points);
        }
    ...
    void G33_auto_tune(float h_fac, float r_fac, float a_fac, const bool stow_after_each, const int8_t verbose_level){
      ...
      SERIAL_EOL();
      probe_G33_points(z_at_pt_base, 3, true, stow_after_each);
      print_G33_results(z_at_pt_base, 3, true, verbose_level);
      ...
      LOOP_XYZ(axis) {
        ...
        if (!home_delta())
          return;
        endstops.not_homing();
        probe_G33_points(z_at_pt, 3, true, stow_after_each);
        for (int8_t i=0; i<13; i++) z_at_pt[i] -= z_at_pt_base[i];
        print_G33_results(z_at_pt, 3, true, verbose_level);
        endstop_adj[axis] += 1.0;
        switch (axis) {
          case A_AXIS :
      ...
      h_fac *= 0.711; // norm_f : normalize to 1.0 for Kossel mini
      h_fac /= 3.0;
      ...
      LOOP_XYZ(axis) {
        ...
        endstop_adj[(axis+1) % 3] -= 1.0/5.0;
        endstop_adj[(axis+2) % 3] += 1.0/5.0;
      ...
      SERIAL_EOL();
      SERIAL_PROTOCOLPGM("Values loaded in memory");
      SERIAL_EOL();
    ...
    inline void gcode_G33() {
      ...
      if (auto_tune) { //(see: https://github.com/LVD-AC/Marlin-AC/tree/1.1.x-AC/documentation)
        G33_auto_tune(h_factor, r_factor, a_factor, stow_after_each, verbose_level);
        G33_CLEANUP();
        return;
      }
      ...
      do {
        ...
        if ((zero_std_dev < test_precision || iterations <= force_iterations) && zero_std_dev > calibration_precision) {
          ...
          float h_fac = h_factor, r_fac = r_factor, a_fac = a_factor;
 
          #define AGRESIVNESS 0.75 // set value between 0.85 and 0.35; too high overshoots back and forth; too low slows convergence down
          #define ZP(N,I) ((N) * z_at_pt[I])
          #define Z24(I) ZP(24/8.0, I)
          #define Z8(I)  ZP(8 /8.0, I)
          #define Z6(I)  ZP(6 /8.0, I)
          #define Z4(I)  ZP(4 /8.0, I)
          #define Z2(I)  ZP(2 /8.0, I)
          #define Z1(I)  ZP(1 /8.0, I)
          h_fac *= AGRESIVNESS * 1.25;
          r_fac *= AGRESIVNESS;
          a_fac *= AGRESIVNESS * (iterations == 1 ? 0 : 1); // skip tower angle corrections in 1st iteration
          ...
          switch (probe_points) {
            ...
            case 2:
              if (towers_set) { // 4 point calibration (towers) matrix
                e_delta[A_AXIS] = ( Z6(0) +Z8(1) -Z4(5) -Z4(9)) * h_fac;
                e_delta[B_AXIS] = ( Z6(0) -Z4(1) +Z8(5) -Z4(9)) * h_fac;
                e_delta[C_AXIS] = ( Z6(0) -Z4(1) -Z4(5) +Z8(9)) * h_fac;
                r_delta         = (Z24(0) -Z8(1) -Z8(5) -Z8(9)) * r_fac;
              }
              else { // 4 point calibration (opposites) matrix
                e_delta[A_AXIS] = ( Z6(0) -Z8(7) +Z4(11) +Z4(3)) * h_fac;
                e_delta[B_AXIS] = ( Z6(0) +Z4(7) -Z8(11) +Z4(3)) * h_fac;
                e_delta[C_AXIS] = ( Z6(0) +Z4(7) +Z4(11) -Z8(3)) * h_fac;
                r_delta         = (Z24(0) -Z8(7) -Z8(11) -Z8(3)) * r_fac;
              }
              break;

            case 3: // 7 point calibration (towers & opposites) matrix
              e_delta[A_AXIS] = ( Z6(0) +Z4(1) -Z2(5) -Z2(9) -Z4(7) +Z2(11) +Z2(3)) * h_fac;
              e_delta[B_AXIS] = ( Z6(0) -Z2(1) +Z4(5) -Z2(9) +Z2(7) -Z4(11) +Z2(3)) * h_fac;
              e_delta[C_AXIS] = ( Z6(0) -Z2(1) -Z2(5) +Z4(9) +Z2(7) +Z2(11) -Z4(3)) * h_fac;
              r_delta         = (Z24(0) -Z4(1) -Z4(5) -Z4(9) -Z4(7) -Z4(11) -Z4(3)) * r_fac;

              if (towers_set) { // 7 point tower angle calibration (towers & opposites) matrix
                t_delta[A_AXIS] = (        +Z8(1) -Z8(5)         -Z8(11) +Z8(3)) * a_fac;
                t_delta[B_AXIS] = (-Z8(9)         +Z8(5) +Z8(7)          -Z8(3)) * a_fac;
                t_delta[C_AXIS] = ( Z8(9) -Z8(1)         -Z8(7) +Z8(11)        ) * a_fac;
              }
              break;

            default: // 13 point calibration matrix
              e_delta[A_AXIS] = ( Z6(0) +Z2(1) -Z1(5) -Z1(9) -Z2(7) +Z1(11) +Z1(3)         +Z2(2) -Z2(6)        -Z2(8) +Z2(12)) * h_fac;
              e_delta[B_AXIS] = ( Z6(0) -Z1(1) +Z2(5) -Z1(9) +Z1(7) -Z2(11) +Z1(3) -Z2(10)        +Z2(6) +Z2(4)        -Z2(12)) * h_fac;
              e_delta[C_AXIS] = ( Z6(0) -Z1(1) -Z1(5) +Z2(9) +Z1(7) +Z1(11) -Z2(3) +Z2(10) -Z2(2)        -Z2(4) +Z2(8)        ) * h_fac;
              r_delta         = (Z24(0) -Z2(1) -Z2(5) -Z2(9) -Z2(7) -Z2(11) -Z2(3) -Z2(10) -Z2(2) -Z2(6) -Z2(4) -Z2(8) -Z2(12)) * r_fac;

              if (towers_set) { // 13 point tower angle calibration matrix 
                t_delta[A_AXIS] = (       +Z4(1) -Z4(5)        -Z4(11) +Z4(3)         +Z4(2) -Z4(6)        +Z4(8) -Z4(12)) * a_fac;
                t_delta[B_AXIS] = (-Z4(9)        +Z4(5) +Z4(7)         -Z4(3) -Z4(10)        +Z4(6) -Z4(4)        +Z4(12)) * a_fac;
                t_delta[C_AXIS] = ( Z4(9) -Z4(1)        -Z4(7) +Z4(11)        +Z4(10) -Z4(2)        +Z4(4) -Z4(8)        ) * a_fac;
              }
              break;
          }
        ...
        print_G33_results(z_at_pt, probe_points, towers_set, verbose_level);
