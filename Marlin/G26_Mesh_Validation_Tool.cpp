  ...
  void unified_bed_leveling::G26() {
    ...
    do {
      ...
      if (location.x_index >= 0 && location.y_index >= 0) {
        ...
        if (!position_is_reachable_raw_xy(circle_x, circle_y, true)) continue;
        ...
        for (tmp = start_angle; tmp < end_angle - 0.1; tmp += 30.0) {
          ...
          float x = circle_x + cos_table[tmp_div_30],    // for speed, these are now a lookup table entry
            ...
            if (!position_is_reachable_raw_xy(x, y, true) || !position_is_reachable_raw_xy(xe, ye, true)) continue;
        ...
        if (i < GRID_MAX_POINTS_X) { // We can't connect to anything to the right than GRID_MAX_POINTS_X.
          ...
          if (is_bit_set(circle_flags, i, j) && is_bit_set(circle_flags, i + 1, j)) { // check if we can do a line to the left
            if (!is_bit_set(horizontal_mesh_line_flags, i, j)) {
              ...
              if (position_is_reachable_raw_xy(sx, sy, true) && position_is_reachable_raw_xy(ex, ey, true)) {
